...
export const PrescriptionsEdit = () => {
    const {
        saveButtonProps,
        refineCore: { query, redirect },
        register,
        control,
        formState: { errors },
        getValues,
        ...
        refineCoreProps: {
            meta: {
                select: "*, dispensing(*, batches(id, batch_number, products(id, product_name)))",
            },
        }
    });

    const prescriptionsData = query?.data?.data;

    const { autocompleteProps: patientAutocompleteProps } = useAutocomplete({
        resource: "patients",
        defaultValue: prescriptionsData?.patient_id,
    });
        ...
        try {
            ...
            const { error } = await supabaseBrowserClient.rpc(
                "upsert_prescription_with_items",
                {
                    ...
                    "prescription_id": prescriptionsData?.id
                }
            );
            if (error) {
                return open?.({
    ...
    return (
        <Edit saveButtonProps={{ ...saveButtonProps, onClick: onSaveBtnClick }}>
            <Box
                component="form"
